<?php
/**
 * Plugin simplerun: Give the ability of execute a script and display its output.
 * 
 * @license    GPL 2 (http://www.gnu.org/licenses/gpl.html)
 * @author     Eduardo Gonzalez <e.gonzalezsolares@gmail.com>
 */
 ...
 */
class syntax_plugin_simplerun extends DokuWiki_Syntax_Plugin {
  ...
  // simplerun analyze byself its component; it use only the main tag analyzer by dokuwiki
  ...
  function connectTo($mode) {
    $this->Lexer->addSpecialPattern('<simplerun>.*?</simplerun>',$mode,'plugin_simplerun');
  }
  ...
  function handle($match, $state, $pos, &$handler){
    ...
    $cmds = array();
    // Process all lines from simplerun body
  ...
  function render($mode, &$renderer, $data) {
    global $currentCommand;	// Global variable who keep the unique Id of the simplerun form.
    $currentCommand += 1;
...
//	    $jqueryBlock .= $this->renderJQueryButton("simplerun","ui-icon-check");
//	  break;
//	  case RC_EXP_CANCELBUTTONTEXT:
...
//	};
  //    };
    //  if ($binaryOutput) {
	// TODO
	//$htmlBlock .= $this->renderFormButton('simplerun',$buttonSubmitText,'',"rcDoDownload");
    //  } else {
	//$htmlBlock .= $this->renderFormButton('simplerun',$buttonSubmitText,'',"rcDoLoad");
    //  };
	...
	//$htmlBlock .= $this->renderFormButton('clearDiv',$buttonCancelText,'',"rcDoClear");
	 ...
	 foreach ($data as $element) {
		 switch($element['key']){
			 case RC_EXP_COMMAND:
			 $command = $this->prepareCommand($element['value']);
			 break;
		 }
	 }
	 
      $htmlBlock = "<b>CMD:</b>".$command;
	  unset($outputValue);
	  ...
	  $htmlBlock = "\n<b>OUTPUT:</b>".$_GET['nrec']."<br/>".$result;
